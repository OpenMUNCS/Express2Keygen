...
namespace USAA_Gen2Library
{
  public class AuthenticationVerifier
  {
    ...
    public bool DecryptandVerify(ref string message)
    {
      ...
      string input = rsaCryption.RSADecrypt(this.encPrivateKey, message);
      message = input;
      string str = Regex.Match(input, "(?<=<PlainText>).*?(?=</PlainText>)", RegexOptions.IgnoreCase).Value;
      string strDeformatterData = Regex.Match(input, "(?<=<Signature>).*?(?=</Signature>)", RegexOptions.IgnoreCase).Value;
      rsaCryption.GetHash(str + this.salt, ref empty1);
      return rsaCryption.SignatureDeformatter(this.signPublicKey, empty1, strDeformatterData);
    }
  }
}
